...
```
we do the same for the featured roasters fetching -- the hooks are pretty similar so i will skip it here 

our home component now looks like this. 
```
import React from "react";
import CTAImage from "./home/CTAImage";
import Roaster from "../components/roasters/Roaster";
...
Great -- we now have the "front page" of the application tested. 

Looking at the home componnet, i think it hhas too many responsibilties so i want to split i up some. 

I want to put featured roaster  and featured products into separate conponents like this 
```
import React from "react";
import Roaster from "../components/roasters/Roaster";
import useFetchFeaturedRoasters from "../hooks/useFetchFeaturedRoasters";
...
The products one is very similar so we skip it 
So now our home component is vastly simlified to this 
```
import React from "react";
import CTAImage from "./home/CTAImage";
import banner from "../images/brewer.jpg";
...
And since it added test before this refactor, i just have to run them again to check everything is in order, which it is :) 

And yes i know -- featured products and featured roaster components are really similiar, why dont use make i dynamic one.. I could, but to what benefit? higher complexity? I think this method is very simple and easy to understand, and i  believe that is more værdifult than everything being dry dry dry... 

I think this is good now, lets pick it up the next post, where will start transforming some of the components to typescript. 
